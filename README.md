...
플레이어에게 5초간 **저항 5**와 즉시 회복 29를 부여합니다.
...
*아우라*가 활성화되어 있으면 주변 16블록 범위 내의 모든 크리퍼를 제거합니다.
...
### call_my_pets
늑대 9마리, 고양이 11마리, 앵무새 5마리를 종류별로 소환합니다.
이미 길들여진 상태로 소환되기에 추가로 길들일 필요는 없습니다.

소환된 동물들은 재생 6과 **저항 5**, 화염 저항, 수중 호흡을 가지고 있습니다.
또한, 늑대들은 기본 공격력 100을 가지고 소환됩니다.

이미 소환된 상태에서 함수를 실행할 경우
소환된 동물들을 자신의 위치로 순간이동 시키며
...
이 자이언트를 공격하면 데미지가 얼마인지 측정해서 알려줍니다.
저항으로 흡수한 데미지를 이용하는 것이기 때문에
주변에 저항 효과를 받은 다른 몹이 있다면 부정확할 수 있습니다.
...
### reset_all
자기 자신을 제외한 모든 엔티티를 제거합니다.
이후 생성되는 아이템과 경험치 구슬도 제거합니다.
디스플레이, 상호 작용 등의 엔티티도 모두 제거되니
주의하시기 바랍니다.

또한 모든 플레이어의 상태 효과, 경험치, 발전 과제, 조합법도 제거하고
월드의 시간도 일출(0)로, 날씨도 맑음으로 만듭니다.
재생 중이던 음악과 효과음도 중단합니다.

`call_my_pets`와 `damage_test` 함수도
실행하지 않았던 것처럼 돌아갑니다.
